...
class HaloAPI {
  ...
  /**
   * Sites flagged "This Site is a Stock Location" in Halo — the pickable
   * scope for the valuation report, instead of hardcoding a site name.
   */
  async listStockSites() {
    const out = [];
    let pageNo = 1;
    while (true) {
      const r = await this.apiCall('GET', '/api/Site', { pageinate: true, page_no: pageNo, page_size: 200 });
      out.push(...(r.sites || []));
      if ((r.sites || []).length < 200) break;
      pageNo++;
    }
    return out
      .filter(s => s.isstocklocation)
      .map(s => ({ id: s.id, name: s.clientsite_name || s.name }));
  }

  /**
   * Stock at a single site, valued at cost. Uses /api/Item's stocklocation_id
   * filter to get only the small candidate set (not the full catalogue), then
   * per-item detail lookups (same logic as stocktake extraction) just for those.
   */
  async getStockAtLocation(stocklocationId) {
    const candidates = [];
    let pageNo = 1;
    while (true) {
      const r = await this.apiCall('GET', '/api/Item', {
    ...
    const rows = [];
    for (const c of candidates) {
      const details = await this.getItemDetails(c.id);
      const loc = (details.stocklocations || []).find(l => l.id === stocklocationId);
      if (!loc) continue;
      const isSerialised = loc.item_serialised_assets_in_stock > 0;
      const quantity = isSerialised ? loc.item_serialised_assets_in_stock : (loc.item_quantity_in_stock || 0);
      if (quantity <= 0) continue;
      rows.push({
        itemId: details.id,
        itemName: details.name,
        quantity,
        unitCost: details.costprice || 0,
        value: quantity * (details.costprice || 0)
      });
    }
    return rows;
  }
  ...
  /**
   * Sales Orders with nothing invoiced yet (this Halo instance never
   * part-invoices, so 0% is the only "open" state).
   */
  async listOpenSalesOrders() {
    const out = [];
    let pageNo = 1;
    while (true) {
      const r = await this.apiCall('GET', '/api/SalesOrder', { pageinate: true, page_no: pageNo, page_size: 200 });
      out.push(...(r.salesorders || []));
      if ((r.salesorders || []).length < 200) break;
      pageNo++;
    }
    return out.filter(so => so.percentage_invoiced === 0);
  }

  /**
   * Consigned stock on a single Sales Order — lines carrying consignment_ids
   * with actual issued assets, valued at the line's cost. Only serialised
   * items get consigned in this Halo instance, so this reads consigned_assets
   * length as the quantity.
   */
  async getConsignedStockForSalesOrder(soId) {
    const raw = await this.apiCall('GET', `/api/SalesOrder/${soId}`, { includedetails: true });
    const rows = [];
    for (const line of (raw.lines || [])) {
      const assets = line.consigned_assets || [];
      ...
      rows.push({
        ...
        value: assets.length * (line.costprice || 0),
  ...
  /**
   * Month-end stock valuation: physical stock at the chosen site(s) + consigned
   * stock on open (uninvoiced) Sales Orders. Avoids scanning the full item
   * catalogue by using stocklocation_id filtering and the small open-SO set.
   */
  async getStockValuationReport(stockSiteIds) {
    const sites = await this.listStockSites();
    const siteNameById = new Map(sites.map(s => [s.id, s.name]));

    const rows = [];
    for (const siteId of stockSiteIds) {
      const siteRows = await this.getStockAtLocation(siteId);
      const siteName = siteNameById.get(siteId) || `Site ${siteId}`;
      for (const r of siteRows) rows.push({ ...r, sourceType: 'stock', source: siteName });
    }

    const openSOs = await this.listOpenSalesOrders();
    for (const so of openSOs) {
      const soRows = await this.getConsignedStockForSalesOrder(so.id);
      for (const r of soRows) {
        rows.push({ ...r, sourceType: 'consigned', source: `Open SO #${so.id} (${r.clientName})` });
      }
    }

    const stockValue = rows.filter(r => r.sourceType === 'stock').reduce((s, r) => s + r.value, 0);
    const consignedValue = rows.filter(r => r.sourceType === 'consigned').reduce((s, r) => s + r.value, 0);

    return {
      rows,
      totals: { stockValue, consignedValue, total: stockValue + consignedValue },
